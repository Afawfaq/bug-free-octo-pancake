...
class ChatterFingerprinter:
    ...
    def analyze_time_patterns(self, baseline: Dict) -> Dict:
        ...
        if first_seen and last_seen:
            ...
            # Determine likely device type based on activity
    ...
    def analyze_destination_patterns(self, baseline: Dict) -> Dict:
        ...
        if destination_diversity < 0.3:
            patterns['communication_style'] = 'focused'  # Talks to few devices
            patterns['behavior'] = 'Single-purpose device or IoT'
        elif destination_diversity > 0.7:
            patterns['communication_style'] = 'diverse'  # Talks to many devices
    ...
    def analyze_protocol_behavior(self, baseline: Dict) -> Dict:
        ...
        # Identify common services based on ports
        service_map = {
    ...
    def create_fingerprint(self, device_ip: str, baseline: Dict) -> Dict:
        """Create a unique fingerprint for a device based on its traffic patterns."""
        ...
        if baseline.get('packets_per_second', 0) > 100:
            chatter_signature['anomaly_indicators'].append('Very high packet rate (potential DDoS or data exfiltration)')
        
        return chatter_signature
    
    def fingerprint_all_devices(self, baselines: Dict) -> Dict:
        """Create fingerprints for all devices in the baseline."""
        fingerprints = {}
...
def main():
    ...
    print(f"[+] Device types: {dict(device_types)}")
